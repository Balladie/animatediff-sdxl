...
from ..models_sdxl.unet import UNet3DConditionModel
...
@dataclass
class AnimationXLPipelineOutput(BaseOutput):
    videos: Union[torch.Tensor, np.ndarray]


class AnimationXLPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin):
    def __init__(
        self,
        vae: AutoencoderKL,
        ...
        if latents is None:
            ...
            if init_latents is not None:
                for i in range(video_length):
                    latents[:, :, i, :, :] = self.scheduler.add_noise(init_latents, latents[:, :, i, :, :], noise_timestep)
        else:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if init_image:
            ...
            denoise_timestep = denoise_timestep.repeat(batch_size * num_videos_per_prompt)

            noise_timestep = timesteps_forward[0:1]
            noise_timestep = noise_timestep.repeat(batch_size * num_videos_per_prompt)
        else:
            denoise_timestep = None
            noise_timestep = None
        ...
        if init_image:
            ...
            num_inference_steps = num_inference_steps_img2img
        ...
        for frame_idx in tqdm(range(latents.shape[0])):
            video.append(self.vae.decode(latents[frame_idx:frame_idx+1] / self.vae.config.scaling_factor, return_dict=False)[0])
